variable "baseami_instance_type" {
  type        = string
  description = "The EC2 instance size to use for the base-ami server."
  default     = "t2.medium"
}

variable "baseami_volume_size" {
  type        = number
  description = "The volume size for the base-ami EC2 instance, GiB."
  default     = 25
}
...
variable "baseami_pubkey" {
  type        = string
  description = "The public key to a private key under your control. You will SSH onto the server using this keypair."
}

variable "baseami_spot_type" {
  type        = string
  description = "Whether to launch the base-ami spot instance as a 'persistent' request or a 'one-time' request."
  default     = "one-time"
}
...
variable "metadata_enabled" {
  type        = string
  description = "Whether EC2 instance medata is enabled. 'enabled' or 'disabled'. Use 'metadata_tokens' to decide between v1 or v2 of instance metadata."
  default     = "enabled"
}
